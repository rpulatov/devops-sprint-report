  ...
  const drawTimeline = () => {
        ...
        timelineItem.events.map((timelineItemEvent) => ({
          ...
          content:
            timelineMode === WorkTimelineMode.Overview
              ? truncateMessage(timelineItemEvent.event, 10)
              : `${truncateMessage(
                  timelineItemEvent.changedBy,
                  15
                )}<br>${truncateMessage(timelineItemEvent.event, 15)}`,
          title:
            timelineMode === WorkTimelineMode.Detailed
              ? ""
              : `${timelineItemEvent.changedBy}<br>${timelineItemEvent.event}<br>${timelineItemEvent.changedAt}`,
          className: `vis-${timelineItem.type}`,
    ...
    const itemsIntervals = _timelineData
      ...
      .map((timelineItem) => ({
        ...
        periodEnd: timelineItem.events.reduce(
